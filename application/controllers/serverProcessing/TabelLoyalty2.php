...
class TabelLoyalty2 extends CI_Controller {
 
    function __construct(){
        parent::__construct();
        $this->load->model('serverProcessing/m_tabel_loyalty2');
    }
  
    function get_data($cari)
    {
        $list = $this->m_tabel_loyalty2->get_datatables($cari);
        $data = array(); 
        $no = $_POST['start'];
    ...
    $data[] = $row; 
        ...
        $output = array( 
            "draw" => $_POST['draw'], 
            "recordsTotal" => $this->m_tabel_loyalty2->count_all(), 
            "recordsFiltered" => $this->m_tabel_loyalty2->count_filtered(), 
